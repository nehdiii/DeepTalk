from utils.TTSTransformerUtils import  get_positional_table, get_sinusoid_encoding_table
from .TTSmodules import *
hidden_size = 256
embedding_size = 512
N_FFT =  2048
...
class MelDecoder(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.pos_dropout = nn.Dropout(p=0.1)
        self.alpha = nn.Parameter(th.ones(1))
        self.decoder_prenet = Prenet(N_MEL, num_hidden * 2, num_hidden, p=0.2)
        ...
        self.ffns = clones(FFN(num_hidden), 3)
        self.mel_linear = Linear(num_hidden, N_MEL * outputs_per_step)
    ...
    def forward(self, memory, decoder_input, c_mask, pos):
        ...
        if self.training:
            m_mask = pos.ne(0).type(th.float)
            mask = m_mask.eq(0).unsqueeze(1).repeat(1, decoder_len, 1)
            if next(self.parameters()).is_cuda:
                mask = mask + th.triu(th.ones(decoder_len, decoder_len).cuda(), diagonal=1).repeat(batch_size, 1,
                                                                                                   1).byte()
            else:
                mask = mask + th.triu(th.ones(decoder_len, decoder_len), diagonal=1).repeat(batch_size, 1, 1).byte()
            mask = mask.gt(0)
            zero_mask = c_mask.eq(0).unsqueeze(-1).repeat(1, 1, decoder_len)
            zero_mask = zero_mask.transpose(1, 2)
        else:
            if next(self.parameters()).is_cuda:
                mask = th.triu(th.ones(decoder_len, decoder_len).cuda(), diagonal=1).repeat(batch_size, 1, 1).byte()
            else:
                mask = th.triu(th.ones(decoder_len, decoder_len), diagonal=1).repeat(batch_size, 1, 1).byte()
            mask = mask.gt(0)
...
class Model(nn.Module):
    ...
    def __init__(self):
        ...
        self.decoder = MelDecoder(hidden_size)
...
class ModelPostNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.pre_projection = Conv(N_MEL, hidden_size)
