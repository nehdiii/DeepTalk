...
through Unidecode. For other data, you can modify _characters.
'''
...
N_MEL = 80 #Number of Mel banks to generate
outputs_per_step = 1



def clones(module,N):
  # we copy the module N times
...
class EncoderPrenet(nn.Module):
    ...
    def forward(self, input_):
        ...
        input_ = self.dropout1(th.relu(self.batch_norm1(self.conv1(input_))))
        input_ = self.dropout2(th.relu(self.batch_norm2(self.conv2(input_))))
        input_ = self.dropout3(th.relu(self.batch_norm3(self.conv3(input_))))
        input_ = input_.transpose(1, 2)
...
class PostConvNet(nn.Module):
    ...
    def __init__(self, num_hidden):
        ...
        self.conv1 = Conv(in_channels=N_MEL * outputs_per_step,
                          out_channels=num_hidden,
                          kernel_size=5,
                          padding=4,
                          w_init='tanh')
    ...
    def forward(self, input_, mask=None):
        ...
        input_ = self.dropout1(th.tanh(self.pre_batchnorm(self.conv1(input_)[:, :, :-4])))
        for batch_norm, conv, dropout in zip(self.batch_norm_list, self.conv_list, self.dropout_list):
            input_ = dropout(th.tanh(batch_norm(conv(input_)[:, :, :-4])))
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, query_mask=None):
        # Get attention score
        attn = th.bmm(query, key.transpose(1, 2))
...
class Attention(nn.Module):
    ...
    def forward(self, memory, decoder_input, mask=None, query_mask=None):
        ...
        result = th.cat([decoder_input, result], dim=-1)
...
class CBHG(nn.Module):
    ...
    def forward(self, input_):
        ...
        for k, (conv, batchnorm) in enumerate(zip(self.convbank_list, self.batchnorm_list)):
            convbank_input = th.relu(batchnorm(self._conv_fit_dim(conv(convbank_input), k + 1).contiguous()))
            convbank_list.append(convbank_input)

        # Concatenate all features
        conv_cat = th.cat(convbank_list, dim=1)

        # Max pooling
        conv_cat = self.max_pool(conv_cat)[:, :, :-1]

        # Projection
        conv_projection = th.relu(self.batchnorm_proj_1(self._conv_fit_dim(self.conv_projection_1(conv_cat))))
...
class Highwaynet(nn.Module):
    ...
    def forward(self, input_):
        ...
        for fc1, fc2 in zip(self.linears, self.gates):
            h = th.relu(fc1.forward(out))
            t_ = th.sigmoid(fc2.forward(out))
